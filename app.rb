...
class Application
  ...
  def print_interface
    @io.puts "\n"
    @io.puts "Please select one of the options:"
  ...
  def user_selection(input)
    @io.puts "You selected option #{input}."
    @io.puts "\n"
    case input
    when "1"
      list_items
    ...
    when "5"
      ...
      @io.puts "Invalid! Please enter valid option."
    end
  end

  def list_items
    @io.puts "ITEMS IN STOCK:"
    @io.puts "\n"
  ...
  def create_item
    ...
    @io.puts "#{item.name} was added in stock."
  end

  def list_orders
    @io.puts "CURRENT ORDERS:"
    @io.puts "\n"
  ...
  def create_order
    ...
    @io.puts "Order for #{order.customer_name} was created."
  end
  ...
  def get_price
    ...
    while item_price.to_f <= 0 # Loops if anything but positive number was entered
      @io.puts "Please enter valid price."
      item_price = @io.gets.chomp
    end
  ...
  def get_quantity
    ...
    while (item_quantity.to_i.to_s != item_quantity) || (item_quantity.to_i <= 0)
      @io.puts "Please enter valid quantity."
      item_quantity = @io.gets.chomp
    end
  ...
  def get_linked_item
    ...
    until id_list.include?(item)
      @io.puts "Please enter valid item in stock."
      item = @io.gets.chomp
    end
