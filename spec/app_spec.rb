...
RSpec.describe Application do
  ...
  it "Loops until valid option selected and then exits" do
    ...
    expect(io).to receive(:puts).with("\n")
    expect(io).to receive(:puts).with("Please select one of the options:")
    ...
    expect(io).to receive(:puts).with("You selected option 6.")
    expect(io).to receive(:puts).with("\n")
    expect(io).to receive(:puts).with("Invalid! Please enter valid option.")
    expect(io).to receive(:puts).with("\n")
    expect(io).to receive(:puts).with("Please select one of the options:")
    expect(io).to receive(:puts).with("1 = list all shop items")
    expect(io).to receive(:puts).with("2 = create a new item")
    ...
    expect(io).to receive(:puts).with("You selected option 5.")
    expect(io).to receive(:puts).with("\n")
  ...
  it "Prints the list of all items in stock" do
    expect(io).to receive(:puts).with("ITEMS IN STOCK:")
    expect(io).to receive(:puts).with("\n")
  ...
  it "Print the list of current orders" do 
    expect(io).to receive(:puts).with("CURRENT ORDERS:")
    expect(io).to receive(:puts).with("\n")
  ...
  it "Creates a new item" do
    ...
    expect(io).to receive(:puts).with("Please enter valid price.")
    ...
    expect(io).to receive(:puts).with("Please enter valid quantity.")
    expect(io).to receive(:gets).and_return("0")
    expect(io).to receive(:puts).with("Please enter valid quantity.")
    expect(io).to receive(:gets).and_return("1")
    expect(io).to receive(:puts).with("Test item was added in stock.")
  ...
  it "Creates a new order linked to item" do
    ...
    expect(io).to receive(:puts).with("Please enter valid item in stock.")
    expect(io).to receive(:gets).and_return("3")
    expect(io).to receive(:puts).with("Order for Test name was created.")
